...
All commands run from this skill directory (or project root with path to script):
...
- `--save` -- save results to drafts (see X_RESEARCH_DRAFTS or ~/clawd/drafts)
...
Use `--save` flag or save manually to drafts (or `~/clawd/drafts/x-research-{topic-slug}-{YYYY-MM-DD}.md`).
...
1. Install [Bun](https://bun.sh) (required for CLI).
2. Get a Composio API key from [composio.dev](https://composio.dev) and connect Twitter.
3. Set `COMPOSIO_API_KEY` in your environment or in project root `.env`.
4. Optional: set `COMPOSIO_CONNECTION_ID` if using a specific connected account; otherwise default is used.
5. Optional: set `X_RESEARCH_DRAFTS` to a directory for `--save` output (default: `~/clawd/drafts`).

## File Structure

```
skills/composio-twitter/
├── SKILL.md           (this file)
