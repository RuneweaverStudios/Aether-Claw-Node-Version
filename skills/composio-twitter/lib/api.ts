...
function getComposioKey(): string {
  ...
  // Try project root .env (when run from skills/composio-twitter or skills/composio-twitter/lib)
  const possibleRoots = [
    join(import.meta.dir, "..", ".."), // from lib/ -> project root
    join(import.meta.dir, "..", "..", ".."), // from lib/ when skill is nested
    process.cwd(),
  ];
  for (const root of possibleRoots) {
    ...
    if (existsSync(envPath)) {
      try {
        ...
        const match = content.match(/COMPOSIO_API_KEY=["']?([^"'\s]+)/m);
  ...
  throw new Error(
    "COMPOSIO_API_KEY not set. Set it in env, project root .env, or ~/.openclaw/.env"
  );
}
