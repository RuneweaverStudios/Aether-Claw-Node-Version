...
This skill uses **Composio** (not direct X API). Set `COMPOSIO_API_KEY` and connect Twitter in Composio dashboard.

Direct X API uses bearer token from env var `X_BEARER_TOKEN`:
...
### Rate Limits (Composio)

Composio free tier: 20,000 API calls/month. Same data as X API, $0.

### Cost (This skill: $0 via Composio)

X API direct uses pay-per-use; this skill uses Composio so there is no per-tweet cost.
...
Same fields/expansions params. This skill fetches by search when needed.
